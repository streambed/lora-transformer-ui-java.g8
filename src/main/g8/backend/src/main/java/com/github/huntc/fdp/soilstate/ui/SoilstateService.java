...
/**
 * Manages services in relation to soil state.
 */
class SoilstateService {
    /**
     * Subscribe to the soilstate events queue returning a source of events. We always start at the tail of
     ...
     */
    static Source<Tuple2<SoilStateReading, Long>, NotUsed> events(DurableQueue durableQueue,
        ...
        return Source.fromSourceCompletionStage(
                durableQueue.source(SoilStateReading.DATA_UP_JSON_TOPIC, Option.empty(), true).<DurableQueue.Received, NotUsed>asJava()
                        ...
                        .thenApply(lastEvents -> {
                            ...
                            return Source
                                    .from(lastEvents)
                                    .concat(
                                            durableQueue.source(SoilStateReading.DATA_UP_JSON_TOPIC, OptionConverters.toScala(lastOffset)).<DurableQueue.Received, NotUsed>asJava()
